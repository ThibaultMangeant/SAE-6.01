...
public class RecuitSimuleCVRP
{
	...
	private double distance( int id1, int id2 ) { return matriceDistance[id1][id2]; }

	/**
	 * Résout le CVRP par recuit simulé.
	 * @param tempInit 					Initial temperature
	 * @param seuilArret 				Minimum temperature
	 * @param alpha 					Coefficient de refroidissement
	 * @param nbIttSansAmelioration 	Nombre d'itérations sans amélioration pour arrêter
	 * @param intervalObservation 		Intervalle pour snapshots
	 * @return 							ResultatRecuit
	 */
	public ResultatRecuit resoudre( double tempInit, double seuilArret, double alpha, 
									int nbIttSansAmelioration, int intervalObservation )
	{
		...
		while ( temperature > seuilArret && iterationsSansAmelioration < nbIttSansAmelioration )
		{
			for ( int i = 0; i < nbIterationsParPalier; i++ )
			{
				...
				if ( delta <= 0 || Math.exp( -delta / temperature ) > random.nextDouble() )
				{
					...
					else { iterationsSansAmelioration++; }
				}
				else { iterationsSansAmelioration++; }
				iteration++;
	...
	private void mouvementRelocate( Solution sol )
	{
		...
		if ( sol.getTournees().get(t1).isEmpty() ) { return; }
	...
	private void mouvement2OptInter( Solution sol )
	{
		...
		if ( route1.size() < 2 || route2.size() < 2 ) { return; }
	...
	private Solution genererSolutionInitiale()
	{
		...
		int 		nbVehiculesMax 	= donnees.getNbVehicules();
		...
		{
			if ( sol.getTournees().size() <= nbVehiculesMax ) { break; }
			...
			if ( chargeI + chargeJ > donnees.getqMax() ) { continue; }
		...
		for ( List<Noeud> r : sol.getTournees() ) { sol.getCharges().add(chargeRoute( sol, r )); }
	...
	{
		for ( List<Noeud> r : sol.getTournees() ) { if ( r.contains( client ) ) { return r; } }
		return null;
	}
	...
	private void calculerDistanceTotale( Solution sol )
	{
		...
		for ( List<Noeud> route : sol.getTournees() )
		{
			...
			total += distance( 0, route.get(0).id );
			for ( int i = 0; i < route.size() - 1; i++ ) { total += distance( route.get(i).id, route.get(i + 1).id ); }
			total += distance( route.get(route.size() - 1 ).id, 0);
		}
